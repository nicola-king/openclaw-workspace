...
SELECTION_RULES = {
    ...
    "政治制度": ["rawls", "confucius", "hanfeizi", "habermas", "arendt"],
    "经济分析": ["smith", "marx", "weber", "kahneman"],
    "市场竞争": ["smith", "sunzi", "kahneman"],
    "组织管理": ["weber", "simon", "tokugawa"],
}
...
def get_statistics() -> Dict:
    """获取思想家统计信息"""
    western_keys = ["popper", "kant", "nietzsche", "sartre", "hegel", "descartes", "hume", "rawls", "foucault", "derrida", "habermas", "baudrillard", "wittgenstein", "heidegger", "merleau-ponty", "smith", "marx", "weber", "camus", "arendt"]
    eastern_keys = ["zhuangzi", "confucius", "laozi", "wangyangming", "nagarjuna", "mencius", "hanfeizi", "sunzi", "tokugawa", "mishima"]
    modern_keys = ["turing", "shannon", "simon", "kahneman", "harari", "bostrom"]
    economic_keys = ["smith", "marx", "weber"]
    
    return {
        "total": len(THINKERS),
        "western": len([k for k in western_keys if k in THINKERS]),
        "eastern": len([k for k in eastern_keys if k in THINKERS]),
        "modern": len([k for k in modern_keys if k in THINKERS]),
        "economic": len([k for k in economic_keys if k in THINKERS]),
    }
...
def main():
    """主函数 - 测试"""
    print("🧠 AGORA 思想家知识库 v2.0 测试")
    print("="*60)
    
    # 统计信息
    stats = get_statistics()
    print(f"\n📊 思想家统计:")
    print(f"   总计：{stats['total']} 位")
    print(f"   西方：{stats['western']} 位")
    print(f"   东方：{stats['eastern']} 位")
    print(f"   现代：{stats['modern']} 位")
    print(f"   经济：{stats['economic']} 位 (交叉分类)")
    ...
    for key, name in new_thinkers.items():
        if key in THINKERS:
            print(f"   ✅ {name}")
        else:
            print(f"   ❌ {name} (缺失)")
    ...
    print("\n✅ AGORA 思想家知识库 v2.0 测试完成!")
    print(f"📚 共收录 {stats['total']} 位思想家")
    print(f"   西方{stats['western']}位 | 东方{stats['eastern']}位 | 现代{stats['modern']}位")
