...
class SmartAIRouter:
    ...
    def analyze_task(self, task_description: str) -> Dict[str, Any]:
        ...
        if task_lower.startswith('claude:'):
            task_type = TaskType.CLAUDE
        # 任务类型识别
        elif any(kw in task_lower for kw in ['代码', 'code', '编程', 'debug', '函数', 'class', 'def ']):
        ...
        if task_type == TaskType.SIMPLE or (task_length < 30 and task_type not in [TaskType.CODE, TaskType.TECHNICAL]):
            complexity = Complexity.LOW
        elif task_length < 1000 and task_type not in [TaskType.COMPLEX]:
    ...
    def select_model(self, task_description: str) -> str:
        ...
        规则:
        1. claude: 前缀 → Claude Code (最高优先级，显式指定)
        2. 已付费额度优先 (百炼)
        3. 免费额度充分利用 (Gemini)
        4. 本地备用 (qwen2.5:7b)
        5. 付费高端仅特殊场景 (Claude/GPT)
        ...
        # 显式 claude: 前缀 → Claude Code
        if task_type == TaskType.CLAUDE:
            logger.info("显式 claude: 前缀 → Claude Code")
            return 'claude'
