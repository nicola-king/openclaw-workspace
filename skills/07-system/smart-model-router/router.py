...
class SmartRouter:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # 海外代理路由器
        from routers.overseas_proxy_router import OverseasProxyRouter
        self.proxy_router = OverseasProxyRouter(self.config_path)
    ...
    def get_model_info(self, model: str) -> Dict[str, Any]:
        ...
        for key, provider in self.OVERSEAS_MODELS.items():
            if key in model.lower():
                # 获取智能分流代理配置
                proxy_config = self.proxy_router.get_proxy_for_ai(provider, model)
                ...
                return {
                    ...
                    'proxy_endpoint': proxy_config.get('endpoint') if proxy_config.get('enabled') else None,
                    'proxy_region': proxy_config.get('region') if proxy_config.get('enabled') else None
                }
