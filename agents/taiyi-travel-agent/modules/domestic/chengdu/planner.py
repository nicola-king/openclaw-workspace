...
class ChengduPlanner:
    ...
    def plan_trip(self, origin: str, destination: str = "成都",
                 start_date: str = "2026-05-01", end_date: str = "2026-05-04",
                 budget: float = 3000, travelers: int = 1,
                 preferences: Optional[List[str]] = None) -> Dict:
        print(f"🐼 成都行程规划：{origin} → {destination}")
        ...
        attractions = {
            "休闲美食": [
                {"name": "锦里", "duration": "2h", "cost": 0},
                {"name": "春熙路", "duration": "3h", "cost": 0},
                {"name": "宽窄巷子", "duration": "2h", "cost": 0},
            ],
            "文化体验": [
                {"name": "武侯祠", "duration": "2h", "cost": 50},
                {"name": "杜甫草堂", "duration": "2h", "cost": 60},
            ],
            "自然风光": [
                {"name": "大熊猫基地", "duration": "3h", "cost": 55},
                {"name": "都江堰", "duration": "1d", "cost": 80},
            ],
        }
...
def main():
    ...
    plan = planner.plan_trip("北京", "成都", "2026-05-01", "2026-05-04", budget=3000, travelers=2)
    planner.save_plan(plan, "chengdu_plan")
