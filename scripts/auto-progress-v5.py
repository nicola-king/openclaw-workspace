#!/usr/bin/env python3
"""
自动进度汇报脚本 v5 - 使用 sessions_send API
每 5 分钟自动发送进度更新到微信
"""
...
SESSIONS_API = "http://localhost:8848/api/sessions/send"  # OpenClaw Gateway API

def log(message):
    with open(LOG_FILE, "a") as f:
        f.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {message}\n")

def send_via_sessions_send():
...
def main():
    log("🚀 自动进度汇报 v5 (sessions_send) 启动")
    
    # 立即发送一次
    log("发送首次汇报...")
    send_via_sessions_send()
