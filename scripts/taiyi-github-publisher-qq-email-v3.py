#!/usr/bin/env python3
"""
太一 GitHub 发布系统 - QQ 邮箱捆绑记忆存储 v3.0
完全智能自动化 · 配置永久存储 · 邮件自动通知
...
class TaiyiGitHubPublisherWithQQEmail:
    """太一 GitHub 发布系统 - QQ 邮箱捆绑版"""
    ...
    def __init__(self):
        ...
        print("=" * 70)
        print("太一 GitHub 发布系统 - QQ 邮箱捆绑记忆存储 v3.0")
        print("=" * 70)
    ...
    def setup_qq_email(self):
        """设置 QQ 邮箱"""
        print("\n📧 设置 QQ 邮箱配置")
        print("-" * 70)
        
        # 获取发件人邮箱
        print("\n请输入 QQ 邮箱地址 (默认：7073481596@qq.com):")
        sender_email = input("邮箱地址：").strip()
        if not sender_email:
            sender_email = "7073481596@qq.com"
        
        # 获取授权码
        print("\n请输入 QQ 邮箱授权码:")
        print("提示：授权码需要在 QQ 邮箱设置中获取")
        print("步骤：QQ 邮箱 → 设置 → 账户 → 开启 SMTP 服务 → 生成授权码")
        sender_password = getpass.getpass("授权码：").strip()
        
        # 保存配置
        self.email_memory['email_config']['sender_email'] = sender_email
        self.email_memory['email_config']['sender_password'] = sender_password
    ...
    def show_status(self):
        ...
        # 邮箱配置
        sender_email = self.email_memory['email_config'].get('sender_email', '未配置')
        print(f"\n📧 邮箱配置:")
        print(f"   发件人：{sender_email}")
        print(f"   SMTP: {self.smtp_server}:{self.smtp_port}")
...
def main():
    ...
    publisher = TaiyiGitHubPublisherWithQQEmail()
    ...
    if len(sys.argv) > 1:
        ...
        if command == 'status':
            ...
            publisher.setup_qq_email()
        elif command == 'publish':
            publisher.publish_all()
        else:
            print(f"未知命令：{command}")
            print("可用命令：status, setup-email, publish")
    else:
        # 默认执行发布
        publisher.publish_all()
